...
echo -e "calls, active calls, cpu load (%), memory (%), bwtx (kb/s), bwrx(kb/s), interval(seg)" 	> data.csv
	while [ $exitcalls = 'false' ]        
		...
		while [ $exitstep = 'false' ]  
			...
			asterisk -rx"channel originate Local/200@call-test-ext application Playback demo-instruct&silence/10&demo-instruct&silence/10&demo-instruct&silence/10&demo-instruct&silence/10&demo-instruct&silence/10&demo-instruct&silence/10"
		done
		let step=step+1
		let i=i+"$call_step"
